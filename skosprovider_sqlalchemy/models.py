...
concept_hierarchy_collection = Table(
    ...
    Base.metadata,
    Column('concept_id_broader', Integer, ForeignKey('concept.id'), primary_key=True),
    Column('collection_id_narrower', Integer, ForeignKey('concept.id'), primary_key=True),
)
...
class Label(Base):
    ...
    language_id = Column(String(64), ForeignKey('language.id'), nullable=True, index=True)
